...
load_dotenv()
import runners.web.oil_web.appsettings as settings

import logger
logger.setup_log(
    app=settings.APP_NAME,
    filename=os.path.join(settings.LOG_PATH, settings.APP_NAME + "_oil_web.log"),
    use_stream=True,
)

from runners.web.oil_web.prompts import OIL_MARKET, COMMODITY_TREE
...
def run():
    try:
        ...
        logging.info(f"Finished {settings.APP_NAME} oil_web. Duration: {time() - start:.1f}s")
