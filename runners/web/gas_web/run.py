...
load_dotenv()
import runners.web.gas_web.appsettings as settings

import logger
logger.setup_log(
    app=settings.APP_NAME,
    filename=os.path.join(settings.LOG_PATH, settings.APP_NAME + "_gas_web.log"),
    use_stream=True,
)

from runners.web.gas_web.prompts import GAS_MARKET, COMMODITY_TREE
...
def run():
    try:
        ...
        logging.info(f"Finished {settings.APP_NAME} gas_web. Duration: {time() - start:.1f}s")
