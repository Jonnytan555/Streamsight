...
class DbPersistenceHandler:
    ...
    Designed to be used standalone or as a base class:
        ...
        # Standalone
        writer = DbPersistenceHandler(engine, "articles", "dbo", ["source_type", "record_id"])
        inserted = writer.insert_new(df)

        # As a base class
        class ArticleWriter(DbPersistenceHandler):
    ...
    def insert_new(self, df: pd.DataFrame) -> list[dict]:
        ...
        try:
            ...
            logging.info("[DbPersistenceHandler] Inserted %d new rows into %s.%s",
                         len(inserted), self.schema, self.table_name)
            return inserted

        except Exception:
            logging.exception("[DbPersistenceHandler] Failed to insert into %s.%s",
    ...
    def _drop_temp(self, temp: str) -> None:
        try:
            ...
            logging.warning("[DbPersistenceHandler] Failed to drop temp table %s", temp)
